...
df <- 
  df %>%
  select(Ano, Mes = Mês, Prestadora, Cod_IBGE = `Codigo IBGE`, Meta_SMP10 = Medição_Meta_Down_Up_SCM4, Medidas = Medidas_Totais, Coletores) 
...
df$Tipo <- cut(df$pop2017, 
               breaks = c(0,50000,100000,300000,20000000),
...
### Quantidade de municipios que estariam de acordo com a qtde de medidas (168) ###

### Mes ###

df %>% 
  filter(Coletores < 50000, Prestadora != "NEXTEL", is.na(Tipo) == FALSE) %>%
  mutate(DeAcordo = Medidas >= 168) %>%
...
df %>% 
  ...
  mutate(DeAcordo = Medidas_Trimestre >= 168) %>%
...
df %>%
  ...
  mutate(DeAcordo = Medidas_Semestre >= 168) %>%
...
df %>% 
  ...
  group_by(Cod_IBGE, Prestadora, Tipo, Trimestre) %>%
  summarize(Medidas_Trimestre = sum(Medidas)) %>%
  mutate(DeAcordo = Medidas_Trimestre >= 42) %>%
  group_by(Trimestre) %>%
  summarize(Qtde_Cidades = sum(DeAcordo),
            Prop_Cidades = mean(DeAcordo)) %>%
  ggplot(aes(x = Trimestre, y = Prop_Cidades)) +
...
df %>% 
  ...
  group_by(Cod_IBGE, Prestadora, Tipo) %>%
  summarize(Medidas_Semestre = sum(Medidas)) %>%
  mutate(DeAcordo = Medidas_Semestre >= 42) %>%
  group_by(1) %>%
  summarize(Qtde_Cidades = sum(DeAcordo),
            Prop_Cidades = mean(DeAcordo)) %>%
  ggplot(aes(x = 1, y = Prop_Cidades)) +
...
df %>% 
  ...
  group_by(Cod_IBGE, Prestadora, Tipo, Trimestre) %>%
  summarize(Medidas_Trimestre = sum(Medidas)) %>%
  mutate(DeAcordo = Medidas_Trimestre >= 66) %>%
  group_by(Trimestre) %>%
  summarize(Qtde_Cidades = sum(DeAcordo),
            Prop_Cidades = mean(DeAcordo)) %>%
  ggplot(aes(x = Trimestre, y = Prop_Cidades)) +
...
df %>% 
  ...
  group_by(Cod_IBGE, Prestadora, Tipo) %>%
  summarize(Medidas_Semestre = sum(Medidas)) %>%
  mutate(DeAcordo = Medidas_Semestre >= 66) %>%
  group_by(1) %>%
  summarize(Qtde_Cidades = sum(DeAcordo),
            Prop_Cidades = mean(DeAcordo)) %>%
  ggplot(aes(x = 1, y = Prop_Cidades)) +
